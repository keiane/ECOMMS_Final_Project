...
% vectorize a matrix and return its Hermitian transpose
param.vec = @(MAT) MAT(:);
param.vecH = @(MAT) MAT(:).';
...
%defines RIS direction, element spacing (in λ), and number of RIS elements.
param.theta_RS = 0;
param.d_E = 0.5;
param.M = 64; % the number of RIS elements

%defines a function to generate steering vectors for a given angle and array size.
param.get_steer = @(theta, L) exp(1j*2*pi*[0:1:L-1].'*param.d_E*param.vecH(sind(theta+param.theta_RS)));

%sets AP-RIS direction randomly, number of vehicles K, and signal-to-noise ratio.
param.theta_AR = -10+rand(1);
param.K = 3;
SNR_dB = 20;

%defines various distances between AP, RIS, vehicle, and sensor.
param.d_AT = 20;
param.d_TR = 30;
param.d_RS = 3;
param.d_AR = 5;

%defines signal amplitudes with path loss and random phases.
z = 1/(param.d_AT*param.d_TR*param.d_RS)*exp(1j*rand(param.K,1));
q = 1/(param.d_AR*param.d_RS)*exp(1j*rand(1));

%defines vehicle DOAs and number of RIS measurement snapshots.
...
%defines angular grids and continuous domain for spatial estimation.
...
num_snapshots = 10;  % You can increase this if desired
R_all = zeros(param.N, num_snapshots);

for snap_idx = 1:num_snapshots
    % Update z with a new random phase each time (to simulate variability)
    z = 1/(param.d_AT*param.d_TR*param.d_RS)*exp(1j*rand(param.K,1));

    % Interference from AP
    r_q = param.G * param.get_steer(param.theta_AR, param.M) * q;

    % Signal from vehicles via RIS
    r_z = param.G * param.get_steer(param.theta_TR, param.M) * z;
...
%solves atomic norm minimization to estimate the sparse signal vector est_xi.
cvx_begin sdp quiet
...
%performs MUSIC algorithm and computes RMSE of the DOA estimate.
est_x = MUSIConesnapshot(R_all, param);
...
%% --- Method 1: FFT-Based DOA Estimation ---
fft_grid = [-45:0.5:45].';  % coarse but fast
steering_fft = param.get_steer(fft_grid, param.M);  % [M x len(fft_grid)]

% Use mean snapshot as input signal
r_fft = mean(R_all, 2);  % average of multiple snapshots

% Beamforming spatial spectrum
fft_spectrum = abs(steering_fft' * (param.G' * r_fft)).^2;
fft_spectrum = fft_spectrum / max(fft_spectrum);  % normalize
fft_spectrum_db = 20 * log10(fft_spectrum);

% Plot FFT spectrum
figure;
plot(fft_grid, fft_spectrum_db, 'LineWidth', 2);
hold on;
stem(param.theta_TR, zeros(size(param.theta_TR)), 'o', 'BaseValue', min(fft_spectrum_db)-5, 'LineWidth', 2);
xlabel('Angle (deg)');
ylabel('FFT Spatial Spectrum (dB)');
title('FFT-Based DOA Estimation');
legend('FFT Spectrum', 'Ground-truth DOA');
grid on;

% RMSE computation
est_spectrum_fft = [fft_grid, fft_spectrum];
rmse_fft = get_rmse(est_spectrum_fft, param.theta_TR);
fprintf('RMSE for FFT method: %.4f degrees\n', rmse_fft);
